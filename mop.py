...
def new_log(text, class_, db_path):
    log_db = shelve.open(db_path + 'mop')  # Open Db

    t_log_dict = log_db['log']  # Get Dict
    log_db['log_num'] = int(log_db['log_num']) + 1  # Update Log Id
    t_log_dict[int(log_db['log_num'])] = (text, class_)  # Insert Log
    log_db['log'] = t_log_dict  # Update Log

    log_db.close()  # Close Db
...
if __name__ == '__main__':
    ...
    if args.init:
        ...
        if args.init[0] == 'update' and mop_db_file:
            ...
            if old_version < 1.41:
                print('\033[1;34m' + '=>' + '\033[0m' + ' 1.4.1')  # 输出当前更新
            ...
            if old_version < 1.42:
                print('\033[1;34m' + '=>' + '\033[0m' + ' 1.4.2')  # 输出当前更新

                t_dict = mop_db['develop']
                t_dict['json2db'] = False
                mop_db['develop'] = t_dict

                new_log(f'{old_version}Update=>{VERSION}', 'system', mop_db_path)
    ...
    if args.install and mop_db_file:
        ...
        for packet_name in down_plugin_list:
            ...
            if bool(mop_db['develop']['python_security_check']):  # 获取用户是否开启python代码安全检查
                ...
                t_dict = mop_db['log']
                mop_db['log_num'] = int(mop_db['log_num']) + 1
                t_dict[int(mop_db['log_num'])] = (
                    f'Find Error > {packet_name}: Warning: {warning}, Error: {error}', 'safe')
                mop_db['log'] = t_dict
